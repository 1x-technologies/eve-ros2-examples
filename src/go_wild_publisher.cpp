...
const double DEFAULT_HEIGHT_ = 0.7;
const double SQUAT_HEIGHT_ = 0.7;
const int TIME_INCREMENT_ = 2;
...
private:
  ...
  void publish_trajectory(unique_identifier_msgs::msg::UUID uuid_msg)
  {
    ...
    trajectory_msg.trajectory_points.push_back(default_target_(t+=1));
    ...
    trajectory_msg.trajectory_points.push_back(default_target_(t+=TIME_INCREMENT_));
...
int main(int argc, char * argv[])
{
  ...
  rclcpp::spin(std::make_shared<Go_Wild_Publisher>());
  rclcpp::shutdown();
  return 0;
}
